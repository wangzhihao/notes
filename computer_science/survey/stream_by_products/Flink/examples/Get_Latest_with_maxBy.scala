...
case class Inventory(merchantId: String, marketplaceId: String, fnsku: String, quantity: Long, eventTime: Timestamp)

val inventory = senv.socketTextStream("localhost", 9234, '\n').map(x => {
    val split = x.split(",")
    Inventory(split(0), split(1), split(2), split(3).toLong, toTimestamp(split(4)))
})
inventory.keyBy("merchantId", "marketplaceId", "fnsku").maxBy("eventTime").print()
